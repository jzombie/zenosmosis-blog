...
2. **No Safety Net:** If you use `read-tree` on top of existing work, the files from the source branch will overwrite files in your index, potentially discarding your unstaged changes without warning. *So long as you start with a clean state, without any staged or uncommited changes, you can easily roll back before committing the new code.*  
